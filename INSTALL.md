...
> **⚠️ 请按以下步骤安装指定版本**（如 `v2.0.0`）。
>
> **v2.0+ 需要 Worker API 已部署。** 参见 SKILL.md 的 Worker deployment 段。
...
Agent 会自动：
1. 拉取仓库最新 tag 的代码（`git clone --branch <latest-tag> --depth 1`）
2. 运行 `skill.yaml` 的元数据检查
3. 安装到 `skills/巴黎评论员/`
4. 运行验证脚本确认安装成功

> 💡 **给 Agent 的补充说明**：如果 Agent 不熟悉 `git clone --branch`，可以要求它"使用最新 tag 拉取"或"检出最新标签"。
...
```bash
# 克隆最新 tag（请先查 VERSIONS.md 或 GitHub tags 页面确认最新版本号）
...
**说明：**
- `--branch <latest-tag>`：只拉取指定 tag 的代码（默认是 main 分支，**不要用**）
...
在浏览器中打开下面这个 URL（替换 `<latest-tag>` 为最新版本号）：
...
> ⚠️ 使用 `git checkout <tag>` 而不是 `git pull origin main`，以免拉取到未发布版本。
