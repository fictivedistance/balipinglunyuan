...
基于简体中文版《巴黎评论》系列构建的作家关系知识图谱，支持自然语言查询访谈状态、作家关系、排行榜、社群、故事路径。
...
**两者的关系：**
- 🏛️ **作家宇宙网站**负责**浏览、探索、发现**——适合慢慢看、随机踩点、发现新关联
- 💬 **巴黎评论员技能**负责**快速问答**——适合明确查某个作家、某种关系、某个列表
- 数据来自同一份 v283 RC 版本的图谱，技能是网站的子集+文本化接口
...
| 👥 **社群发现** | 算法识别的作家社群成员查询 |
| 📚 **故事路径** | 8条预设主题关系链（现代主义、拉美、女性等） |

---

## 📦 安装

### 方式1：作为 OpenClaw 技能安装
```bash
# 将整个目录复制到 OpenClaw skills 目录
cp -r 巴黎评论员 ~/.openclaw/workspace/skills/
```
...
### 4. 社群查询
**什么是社群？**
社群是算法自动识别的"作家朋友圈"——联系紧密、经常被同时提到的作家会被分到同一个社群。每个社群通常代表一个流派、时代或地域的作家群体（比如"拉美文学爆炸""美国现代主义"等）。社群编号是算法自动分配的。
...
社群由社区发现算法自动生成，每个社群代表一组联系紧密的作家群体。

---

### 5. 故事路径
**什么是故事路径？**
故事路径是围绕特定主题整理的作家引用链条。
每条路径都提取了某个主题下的核心作家和他们之间的相互评价，帮你快速了解一个文学话题的全貌。
...
python3 scripts/nl_interface.py "海明威和福克纳有什么关系"
python3 scripts/nl_interface.py "拉美故事路径"
```
...
# 社群查询
python3 scripts/paris_network_query.py community --community-id 10

# 故事路径
python3 scripts/paris_network_query.py story-path --key "拉美"
```
...
《巴黎评论》访谈内容版权归《巴黎评论》杂志和简体中文版《巴黎评论》系列编辑部所有。本技能仅用于研究和学习目的，请勿擅自商用。
