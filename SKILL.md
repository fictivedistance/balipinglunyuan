...
## ⚠️ 输出规则（所有 Agent 必须遵守）

调用 `scripts/nl_interface.py` 后，如果输出以 `[[FMT_LOCK]]` 开头，**必须将脚本 stdout 原样转发给用户**，不得摘要、改写、简化或重新组织格式。脚本输出即为最终回复内容。

- ❌ 不得删除表格、引用块、链接或标准结尾段
- ❌ 不得将表格改为短文本
- ❌ 不得省略 `[[FMT_LOCK]]` 标记之后的部分输出
- ✅ 看到标记即原样转发，一个字都不改

**原因：** 脚本已内置完整的格式化逻辑（v2.2.3），所有表格、引用块、标准结尾段均在脚本层生成。Agent 自行改写会导致跨 Agent 输出不一致。
